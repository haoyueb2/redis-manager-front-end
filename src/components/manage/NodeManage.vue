<template>
  <div id="cluster-manage" class="body-wrapper">
      ...
      <div class="base-info-operation-wrapper">
        ...
        <span class="base-info-item">
          ...
          <el-tag size="mini" v-if="cluster.installationEnvironment == 0"
            >Docker</el-tag
          >
          <el-tag size="mini" v-else-if="cluster.installationEnvironment == 1"
            >Machine</el-tag
          >
          <el-tag size="mini" v-else-if="cluster.installationEnvironment == 3"
            >Humpback</el-tag
          >
        </span>
        <span
          class="base-info-item"
          v-if="cluster.image != null && cluster.image != ''"
        >
          Image:
      ...
      <el-table :data="sentinelMasterList">
        ...
        <el-table-column property="status" label="Status">
          <template slot-scope="scope">
            <el-tag size="mini" v-if="scope.row.status == 'ok'">{{
              scope.row.status
            }}</el-tag>
        ...
        <el-table-column prop="monitor" label="Monitor" align="center" sortable>
          <template slot-scope="scope">
            <i
              class="el-icon-success status-icon normal-status"
              v-if="scope.row.monitor"
            ></i>
        ...
        <el-table-column label="Master Node">
          <template slot-scope="scope"
            >{{ scope.row.host }}:{{ scope.row.port }}</template
          >
        </el-table-column>
        <el-table-column
          property="lastMasterNode"
          label="Last Master Node"
        ></el-table-column>
        <el-table-column
          property="numSlaves"
          label="Num Slaves"
          ...
          <template slot-scope="scope">
            ...
            >
              ...
              <el-dropdown-menu slot="dropdown">
                <el-dropdown-item
                  @click.native="
                    sentinelMasterInfoVisible = true;
                    sentinelMaster = scope.row;
                  "
                  >Detail</el-dropdown-item
                >
                <el-dropdown-item
                  @click.native="
                    sentinelMasterSlavesVisible = true;
                    sentinelMaster = scope.row;
                  "
            ...
            <el-dropdown size="mini" split-button type="danger" trigger="click">
              Operate
              <el-dropdown-menu slot="dropdown">
                <el-dropdown-item @click.native="editSentinelMaster(scope.row)"
                  >Edit</el-dropdown-item
                >
                <el-dropdown-item
                  @click.native="
                    failoverSentinelMasterVisible = true;
                    sentinelMaster = scope.row;
                  "
                ...
                <el-dropdown-item
                  @click.native="
                    deleteSentinelMasterVisible = true;
                    sentinelMaster = scope.row;
                  "
        ...
        <div style="display: flex; justify-content: space-between;">
            ...
            <span
              v-if="
                cluster.redisMode == 'cluster' &&
                  !cluster.initialized &&
                  cluster.clusterSlotsAssigned == 0
              "
            >
              <el-divider direction="vertical"></el-divider>
              <el-link
          ...
          <div style="display: flex;align-items: center;">
            ...
            <el-link
              ...
              type="primary"
              @click="
                sentinelMasterEditVisible = true;
        ...
        >
          ...
          <el-table-column label="Link State" sortable width="150px">
            <template slot-scope="scope">
              ...
              >
              <el-tag size="small" class="pointer" v-else type="warning">{{
                scope.row.linkState
              }}</el-tag>
            </template>
          </el-table-column>
          <el-table-column label="Flags" width="90px">
            <template slot-scope="scope">
              <el-tag size="mini" v-if="scope.row.flags == 'master'">
                {{ scope.row.flags }}
                <span v-if="scope.row.replicaNumber > 0"
                  >[{{ scope.row.replicaNumber }}]</span
                >
              </el-tag>
              <el-tag size="mini" class="pointer" type="info" v-else>{{
                scope.row.flags
              }}</el-tag>
            </template>
          </el-table-column>
          ...
          >
            <template slot-scope="scope">
              {{ scope.row.slotRange }}
              <el-tag size="mini" v-if="scope.row.slotRange != null">{{
                scope.row.slotNumber
              }}</el-tag>
            </template>
          </el-table-column>
        ...
        <el-table-column label="Operation" width="120px">
          <template slot-scope="scope">
            <el-button
              ...
              :disabled="
                (operationNode.nodeId == scope.row.nodeId ||
                  operationNode.masterId == scope.row.nodeId) &&
                  operationNode.inCluster
              "
    ...
    <el-dialog
      title="Fail Over"
      :visible.sync="failOverVisible"
      width="30%"
      v-if="failOverVisible"
    >
      <span
        >{{ operationNode.host }}:{{ operationNode.port }} will become the
        master</span
      >
      <span slot="footer" class="dialog-footer">
        <el-button size="small" @click="failOverVisible = false"
          >Cancel</el-button
        >
        <el-button size="small" type="warning" @click="failOver()"
          >Fail Over</el-button
        >
      </span>
    </el-dialog>
    ...
    >
      ...
      <div slot="footer" class="dialog-footer">
        <el-button size="small" @click="importNodeVisible = false"
          >Cancel</el-button
        >
        <el-button
          size="small"
          type="primary"
          @click="importNode('newRedisNode')"
          >Confirm</el-button
        >
      </div>
    </el-dialog>
    ...
    >
      ...
      <div slot="footer" class="dialog-footer">
        <el-button size="small" @click="editConfigVisible = false"
          >Cancel</el-button
        >
        <el-button
          size="small"
          type="primary"
          @click="editConfig('redisConfig')"
          >Confirm</el-button
        >
      </div>
    </el-dialog>
    ...
    >
      <span
        >{{ operationNode.host }}:{{ operationNode.port }} will be forget</span
      >
      <span slot="footer" class="dialog-footer">
        <el-button size="small" @click="forgetVisible = false"
          >Cancel</el-button
        >
        <el-button size="small" type="warning" @click="forget()"
          >Forget</el-button
        >
      </span>
    </el-dialog>
    ...
    >
      ...
      <div slot="footer" class="dialog-footer">
        <el-button size="small" @click="moveSlotVisible = false"
          >Cancel</el-button
        >
        <el-button size="small" type="primary" @click="moveSlot('slotRange')"
          >Confirm</el-button
        >
      </div>
    </el-dialog>
    ...
    >
      <span
        >{{ operationNode.host }}:{{ operationNode.port }} will be start</span
      >
      <span slot="footer" class="dialog-footer">
        <el-button size="small" @click="startNodeVisible = false"
          >Cancel</el-button
        >
        <el-button size="small" type="primary" @click="startNode()"
          >Start</el-button
        >
      </span>
    </el-dialog>
    ...
    >
      <span
        >{{ operationNode.host }}:{{ operationNode.port }} will be stop</span
      >
      <span slot="footer" class="dialog-footer">
        <el-button size="small" @click="stopNodeVisible = false"
          >Cancel</el-button
        >
        <el-button size="small" type="danger" @click="stopNode()"
          >Stop</el-button
        >
      </span>
    </el-dialog>
    ...
    >
      <span
        >{{ operationNode.host }}:{{ operationNode.port }} will be restart</span
      >
      <span slot="footer" class="dialog-footer">
        <el-button size="small" @click="restartNodeVisible = false"
          >Cancel</el-button
        >
        <el-button size="small" type="primary" @click="restartNode()"
          >Restart</el-button
        >
      </span>
    </el-dialog>
    ...
    >
      <span
        >{{ operationNode.host }}:{{ operationNode.port }} will be delete</span
      >
      <span slot="footer" class="dialog-footer">
        <el-button size="small" @click="deleteNodeVisible = false"
          >Cancel</el-button
        >
    ...
    >
      ...
      <span slot="footer" class="dialog-footer">
        <el-button size="small" type="primary" @click="initSlots()"
          >Init</el-button
        >
      </span>
    </el-dialog>
      ...
      <div>
        ...
        >
          <el-form-item label="Master Name" prop="name">
            <el-tag size="small" v-if="isUpdateSentinelMaster">{{
              sentinelMaster.name
            }}</el-tag>
            <el-input
              v-model="sentinelMaster.name"
              maxlength="30"
              show-word-limit
              v-else
            ></el-input>
          ...
          <el-form-item
            ...
            prop="downAfterMilliseconds"
          >
            <el-input
              v-model.number="sentinelMaster.downAfterMilliseconds"
            ></el-input>
          </el-form-item>
          ...
          <el-form-item label="Failover Timeout" prop="failoverTimeout">
            <el-input
              v-model.number="sentinelMaster.failoverTimeout"
            ></el-input>
          </el-form-item>
    ...
    >
      <span
        >{{ sentinelMaster.host }}:{{ sentinelMaster.port }} will be not
        monitored</span
      >
      <span slot="footer" class="dialog-footer">
        <el-button size="small" @click="deleteSentinelMasterVisible = false"
          >Cancel</el-button
        >
        <el-button size="small" type="danger" @click="deleteSentinelMaster()"
          >Delete</el-button
        >
      </span>
    </el-dialog>
    ...
    >
      <span
        >{{ sentinelMaster.host }}:{{ sentinelMaster.port }} will failover</span
      >
      <span slot="footer" class="dialog-footer">
        <el-button size="small" @click="failoverSentinelMasterVisible = false"
          >Cancel</el-button
        >
        <el-button size="small" type="danger" @click="failoverSentinelMaster()"
          >Failover</el-button
        >
      </span>
    </el-dialog>
